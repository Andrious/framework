...
public class dbHelper extends SQLiteOpenHelper{
    ...
    //    @Keep
    private static final String DOWNGRADEFILE_PREFIX = "downgrade-";
    ...
    public dbHelper(Context controller, dbDataRecords rec, String dbFile,
            SQLiteDatabase.CursorFactory factory, int version, DatabaseErrorHandler errorHandler){

        super(controller, dbFile, factory, version, errorHandler);

        mdbRecords = rec;

        rec.setDbHelper(this);
    ...
    public boolean save(appModel.dataItem itemToDo){
    ...
    public void bindRecValues(String columnName, String value){
        ...
        mdbRecords.bindRecValues(columnName, value);
    }
    ...
    private int updateRec(appModel.dataItem itemToDo){

        return updateRec(mdbRecords.bindRecValues(itemToDo), itemToDo.getId());
    }
    ...
    private int updateRec(ContentValues recValues, long id){
        ...
        try{
            ...
            result = mDB.update(mdbRecords.DATABASE_NAME(), recValues,
                    mdbRecords.DBKEY_FIELD() + " = ?",
    ...
    private long insertRec(appModel.dataItem itemToDo){

        return insertRec(mdbRecords.bindRecValues(itemToDo));
    }
    ...
    private long insertRec(ContentValues recValues){
        ...
        try{
            ...
            result = mDB.insert(mdbRecords.DATABASE_NAME(), null, recValues);
...
//    }
    ...
    public boolean createTable(){
        ...
        try{
            ...
            mDB.execSQL(mdbRecords.DATABASE_CREATE());
    ...
    @Override
    // Called in getDatabaseLocked(boolean writable) when a database is first opened.
    // Break here to see the created script for example.
    ...
    public boolean execSqlFile(String sqlFile, SQLiteDatabase db)
            throws SQLException{
        ...
        try{
            ...
            for (String sqlInstruction : dbSQLParser
                    .parseSqlFile(SQL_DIR + File.separator + sqlFile, this.mContext.getAssets())){
    ...
    public void onDowngrade(SQLiteDatabase db, int oldVersion, int newVersion){
        ...
        execGradeFile(DOWNGRADEFILE_PREFIX, new ExecGrade(db, oldVersion, newVersion){

            boolean validVersion(int fileVersion){

                return fileVersion == mNewVersion;
            }
        });
    }
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion){
        ...
        execGradeFile(UPGRADEFILE_PREFIX, new ExecGrade(db, oldVersion, newVersion){

            boolean validVersion(int fileVersion){

                return (fileVersion > mOldVersion && fileVersion <= mNewVersion);
            }
        });
    }
    ...
    private void execGradeFile(String prefix, ExecGrade obj){
            ...
            for (String sqlFile : dbSQLParser.list(SQL_DIR, this.mContext.getAssets())){
                ...
                if (sqlFile.startsWith(prefix)){
                    ...
                    int fileVersion = Integer.parseInt(
                            sqlFile.substring(prefix.length(),
                                    sqlFile.length() - UPGRADEFILE_SUFFIX.length()));

                      // Stop if any sql file fails
                       if(!obj.execute(sqlFile, fileVersion)){
    ...
    abstract class ExecGrade{
        ...
        ExecGrade(SQLiteDatabase db, int oldVersion, int newVersion){
        ...
        final boolean execute(String sqlFile, int fileVersion){

            boolean exe = true;

            if(validVersion(fileVersion)){

                exe = execSqlFile(sqlFile, mDB);
            }

            return exe;
        }

        abstract boolean validVersion(int fileVersion);
    }
